    ...
    public function store(Request $request)
{
    ...
    $validatedData = $request->validate([
        ...
        'date' => 'required|date',
        'seat_number' => 'required|json', // Ensure seat_number is required and valid JSON
    ]);
    ...
    // Decode the seat_number from JSON
    $seatNumbers = json_decode($request->seat_number, true);

    // Check if the seat number is properly formatted (should be an array and contain at least one element)
    if (!is_array($seatNumbers) || empty($seatNumbers)) {
        return back()->withErrors(['seat_number' => 'Invalid seat selection. Please select a valid seat.']);
    }

    // Check if the seat is already booked (optional, if you want to ensure no duplicate bookings for a seat)
    $seatAlreadyBooked = Booking::where('date', $request->date)
        ->where('seat_number', $seatNumbers[0]) // Assuming only one seat is selected
        ->exists();
    ...
    Booking::create([
        ...
        'date' => $request->date,
        'seat_number' => $seatNumbers[0], // Use the first seat from the JSON array
    ]);
