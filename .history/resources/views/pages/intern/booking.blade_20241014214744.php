...
<div class="container">
    ...
    <div class="form-group text-center">
        <label for="intern_name" class="font-weight-bold">Intern Name:</label>
        <input type="text" id="intern_name" name="intern_name" class="form-control mx-auto" style="max-width: 400px;" required>
        @error('intern_name')
            <div class="error text-danger">{{ $message }}</div> <!-- Display error if validation fails -->
        @enderror
    </div>
        ...
        for (let i = 0; i < numSeats; i++) {
            ...
            seat.addEventListener('click', function () {
                if (!seat.classList.contains('booked')) {
                    ...
                    if (selectedSeat) {
                        selectedSeatInput.val(JSON.stringify([seat.dataset.seatNumber])); // Send seat number as JSON
                    } else {
