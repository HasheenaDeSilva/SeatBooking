...
seat.addEventListener('click', function () {
    if (!seat.classList.contains('booked')) {
        ...
        if (seat.classList.contains('selected')) {
            selectedSeats.push(seat.dataset.seatNumber); // Add to selected seats
        } else {
            selectedSeats = selectedSeats.filter(seatNum => seatNum !== seat.dataset.seatNumber); // Remove from selected seats
        }

        // Update the hidden input with a JSON string
        selectedSeatInput.val(JSON.stringify(selectedSeats));
    }
});
