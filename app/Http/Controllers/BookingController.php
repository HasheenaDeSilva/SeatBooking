   ...
   public function create()
{
    ...
    public function store(Request $request)
    {
        ...
        $booking = Booking::create([
            ...
            'intern_name' => session('intern_name'), // Retrieve intern_name from the session
            'date' => $request->date,
        ...
        try {
            Mail::to($intern->email) // Make sure intern->email has a valid email address
                ->send(new BookingConfirmation($booking)); // Pass your booking data to the Mailable
        } catch (\Exception $e) {
   ...
   public function showBookingPage(Request $request)
    ...
    public function showBookingNewPage()
    {
        return view('pages.intern.bookingnew'); 
    }
    ...
    {
        // Assuming $request->date contains the selected date
        $bookedSeats = Booking::where('date', $request->date)->pluck('seat_number');
